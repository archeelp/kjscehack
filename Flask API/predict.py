...
def predictdisease():
    ...
    for symptom in symptoms:
        temp.append(symptoms_dict[symptom])
    #input_vector[[symptoms_dict['runny_nose'], symptoms_dict['neck_pain']]] = 1
    input_vector[temp] = 1
    result = loaded_model.predict([input_vector])
    return {"result" : result[0]}

@app.route('/suggestdiet', methods=['POST'])
def suggestdiet():
    params = request.json
    name=params['name']
    ...
    phys_act=params['physical_activity']
